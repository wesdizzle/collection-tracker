...
// .wrangler/tmp/bundle-SPPK61/checked-fetch.js
  ...
  async fetch(request, env) {
    ...
    try {
      if (path === "/api/games") {
        ...
        let query = `SELECT * FROM platforms ORDER BY brand ASC, COALESCE(parent_platform_id, id) ASC, launch_date ASC`;
        const stmt = env.DB.prepare(query);
        const { results } = await stmt.all();
        return Response.json(results);
      }
...
// .wrangler/tmp/bundle-SPPK61/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-SPPK61/middleware-loader.entry.ts
