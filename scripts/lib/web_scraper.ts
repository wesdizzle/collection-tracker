...
export async function scrapePriceCharting(
  ...
  try {
    const query = encodeURIComponent(`${title} ${platform}`).replace(
      /%20/g,
      '+',
    );
    ...
    console.log(
      `[Scraper] Initial Search Result HTML (first 1000 chars): ${dataStr.substring(0, 1000)}`,
    );
    ...
    if (dataStr.trim().startsWith('{')) {
      try {
        ...
        if (products.length === 0) {
          console.log(
            `  No results found in JSON on PriceCharting for ${title}.`,
          );
          return null;
        }
        ...
        const productSlug = best.productName
          .toLowerCase()
          .replace(/[^a-z0-9 ]/g, '')
          .replace(/ /g, '-');
        ...
        console.log(
          `  Selected JSON Result: ${best.productName} (${productUrl})`,
        );
      } catch {
    ...
    if ($('#games_table').length > 0) {
      const results = $('#games_table td.title a')
        .map((i, el) => {
      ...
      if (results.length > 0) {
        ...
        console.log(
          `  Selected HTML Result: ${bestMatch.title} (${productUrl})`,
        );
      }
    }
    ...
    if (productUrl !== searchUrl && !productUrl.includes('search-products')) {
      ...
      const productHtml =
        typeof productResponse.data === 'string'
 ...
 */
export async function scrapePlayStationStore(
  title: string,
): Promise<Partial<ScrapedMetadata> | null> {
  ...
  try {
    ...
    const productUrl =
      'https://store.playstation.com' + firstResult.attr('href');

    await delay(500);
    const productResponse = await axios.get(productUrl, { headers: HEADERS });
    const $product = cheerio.load(productResponse.data as string);

    const description = $product('[data-qa="mfe-game-overview#description"]')
      .text()
      .trim();
    const releaseDate = $product('[data-qa="gameInfo#releaseDate#value"]')
      .text()
      .trim();
