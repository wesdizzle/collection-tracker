...
export async function recomputeCanonicalSeries() {
  ...
  const allGamesRaw = db
    .prepare(
      `SELECT ${CONFIG.columns.seriesList} as s, ${CONFIG.columns.franchiseList} as f FROM games WHERE igdb_id IS NOT NULL`,
    )
    .all() as { s: string; f: string }[];
  ...
  for (const game of games) {
    ...
    const applyRedirects = (list: string[]) => {
      return list
        .map((item) => {
          if (SERIES_REDIRECTS[item] !== undefined)
            return SERIES_REDIRECTS[item];
          return item;
        })
    ...
    if (candidates.length > 0) {
      ...
      const survivors = Object.values(groups).map((group) => {
        return group.reduce((prev, curr) =>
          (frequencies[curr] || 0) > (frequencies[prev] || 0) ? curr : prev,
        );
      });
      ...
      const primaryPrefix =
        colonIndex > 0 ? normalize(game.title.substring(0, colonIndex)) : null;
      ...
      const scores = survivors.map((s) => {
        ...
        if (normS === 'xeno') {
          if (normTitle.includes('xenoblade'))
            candidate = 'Xenoblade Chronicles';
          else if (normTitle.includes('xenogears')) candidate = 'Xenogears';
          else if (normTitle.includes('xenosaga')) candidate = 'Xenosaga';
        }

        if (
          series.some((x) => normalize(x) === normS) &&
          franchises.some((x) => normalize(x) === normS)
        )
          score += 5;
      ...
      if (
        scores.length > 0 &&
        (scores[0].score > 1 || (scores[0].score > 0 && scores.length === 1))
      ) {
    ...
    if (updateCount % 500 === 0)
      console.log(`Updated ${updateCount}/${games.length} games...`);
  }
...
async function runVectorTieBreaker(
  ...
  for (const candidate of candidates) {
    const candidateOutput = await embedder(candidate, {
      pooling: 'mean',
      normalize: true,
    });
