...
describe('Worker API Logic', () => {
  ...
  it('GET /api/export/gameye-data returns owned games and toys with GAMEYE IDs and completeness masks', async () => {
    // Insert game and releases (owned CIB, and unowned regional release)
    db.prepare(
      `
      INSERT INTO games (stable_id, id, title, platform_id, gameye_id, gameye_platform_id)
      VALUES (101, 'pokemon-box', 'Pokémon Box', 20, 27462, 2)
    `,
    ).run();
    db.prepare(
      `
      ...
      VALUES
        ('rel-us', 101, 'USA', 1, 1, 1),
        ('rel-eu', 101, 'Europe', 0, 0, 0)
    `,
    ).run();

    // Insert owned toy and unowned toy
    db.prepare(
      `
      INSERT INTO toys (stable_id, name, line, gameye_id, ownership_status)
      VALUES
        (501, 'Spyro', 'Skylanders', 55555, 1),
        (502, 'Bowser', 'amiibo', 66666, 0)
    `,
    ).run();

    const req = new Request('http://localhost/api/export/gameye-data');
    const res = await worker.fetch(req, mockEnv);
    expect(res.status).toBe(200);
    ...
    expect(body.games[0].title).toBe('Pokémon Box');
    expect(body.games[0].gameye_id).toBe(27462);
    expect(body.games[0].has_case).toBe(1);
    expect(body.games[0].has_manual).toBe(1);

    expect(body.toys).toHaveLength(1);
    expect(body.toys[0].title).toBe('Spyro');
    expect(body.toys[0].gameye_id).toBe(55555);
  });
