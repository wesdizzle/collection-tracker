...
- There are still several games on the 3DS and older consoles that have not been properly matched to physical releases in the datfiles. We need to match these.
- `Metal Gear Solid (Japan, Asia) (Disc 1) (Ichi) (Premium Package).cue` and `Metal Gear Solid (Japan, Asia) (Disc 2) (Ni) (Premium Package).cue` should be treated as one multi-disc release. They deviate from the currently supported format in that they use English and Japanese for counting; "ichi" and "ni" instead of "1" and "2".
- `Metal Gear Solid - Special Missions (Europe) (En,Fr,De,Es,It).cue` should be matched to `Metal Gear Solid: VR Missions` instead of the regular `Metal Gear Solid`.
- Alternate releases (Demo, Alt, Beta 2, Rev 1, etc.) should be sorted after the main release if they share the same release date.
